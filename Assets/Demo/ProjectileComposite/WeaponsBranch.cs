...
namespace Demo.ProjectileComposite
{
    public class WeaponsBranch : WeaponsTree
    {
        ...
        public override AnimatorOverrideController Animator(SpellCreator.SpellTypes type) => 
            _children.Where(weapon => weapon.SpellType() == type).Select(weapon => weapon.Animator(type)).FirstOrDefault();
    ...
    class SpellBranch : WeaponsTree
    {
        ...
        public override SpellCreator.SpellTypes GetSpellType(AttackAdapter.AttackType type) => SpellType();
        public override AnimatorOverrideController Animator(SpellCreator.SpellTypes type)
        {
            return _children.Where(weapon => weapon.SpellType() == type).Select(weapon => weapon.Animator(type)).FirstOrDefault();
        }
